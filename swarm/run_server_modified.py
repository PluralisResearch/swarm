...
from src.moe.server import Server
from src.moe.server.layers import schedule_name_to_scheduler
...
from src.moe.server.layers.custom_experts import register_expert_class

ffn_sample_input = lambda batch_size, hid_dim: torch.empty((batch_size, hid_dim))

@register_expert_class("ffn_head", ffn_sample_input)
class FeedforwardBlock(nn.Module):
    def __init__(self, hid_dim):
        super().__init__()
        self.ffn = nn.Linear(hid_dim, int(hid_dim/2))
        self.relu = nn.ReLU()

    def forward(self, x):
        ffn_output = self.ffn(x)
        return self.relu(ffn_output)
    
tail_sample_input = lambda batch_size, hid_dim: (
    torch.empty((batch_size, hid_dim)),
    torch.randint(0, 10, (batch_size,), dtype=torch.long),
)

@register_expert_class("ffn_tail", tail_sample_input)
class FeedforwardBlock(nn.Module):
    def __init__(self, hid_dim):
        super().__init__()
        self.ffn = nn.Linear(hid_dim, int(hid_dim/2))
        self.relu = nn.ReLU()
        self.ffn2 = nn.Linear(int(hid_dim/2), 10)

    def forward(self, hidden_state, labels):
        ffn_output = self.ffn(hidden_state)
        ffn_output = self.relu(ffn_output)
        ffn_output = self.ffn2(ffn_output)
        loss = F.cross_entropy(ffn_output, labels, reduction='none')
        return loss

cnn_vanilla_sample_input = lambda batch_size, hid_dim: (
    torch.empty((batch_size, 1, hid_dim, hid_dim)),
    torch.randint(0, 10, (batch_size,), dtype=torch.long),
)

@register_expert_class("cnn_vanilla", cnn_vanilla_sample_input)
class CNNVanilla(nn.Module):
    ...
    def forward(self, hidden_state, labels):
        ...
        x = F.relu(x)
        x = self.fc2(x)
        loss = F.cross_entropy(x, labels, reduction='none')
        return loss    
    
cnn_head_sample_input = lambda batch_size, hid_dim: torch.empty((batch_size, 1, hid_dim, hid_dim))

@register_expert_class("cnn_head", cnn_head_sample_input)
...
cnn_tail_sample_input = lambda batch_size, hid_dim: (
    torch.empty((batch_size, hid_dim)),
    torch.randint(0, 10, (batch_size,), dtype=torch.long),
)

@register_expert_class("cnn_tail", cnn_tail_sample_input)
class CNNTail(nn.Module):
    def __init__(self, hid_dim):
        super().__init__()
        self.fc1 = nn.Linear(9216, 128)
        self.fc2 = nn.Linear(128, 10)
